...
def query_oauth_by_controler_id(controler_id:str,channel:str):
    with db_client.session_scope as session:
        stmt = select(OauthCredentialsTable).where((OauthCredentialsTable.controler_id == controler_id) & (OauthCredentialsTable.channel == channel))
        oauth_info = session.execute(stmt).scalar_one_or_none()
        logger.info_database('query_oauth_by_controler_id return success!')
...
def query_oauth_by_controler_name(controler_name:str,channel:str):
    with db_client.session_scope as session:
        stmt = select(OauthCredentialsTable).where((OauthCredentialsTable.controler_name == controler_name) & (OauthCredentialsTable.channel == channel))
        oauth_info = session.execute(stmt).scalar_one_or_none()
        logger.info_database('query_oauth_by_controler_name return success!')
...
def update_oauth_info(controler_id:str,controler_name:str,channel:str,oauth_info:dict):
    logger.info_database('update_oauth_info start!')
    ...
    with db_client.session_scope as session:
        ...
        if db_oauth_info:
            db_oauth_info.oauth_credentials = oauth_info
            logger.info_database('update_oauth_info update success!')
            logger.debug(f'update_oauth_info,controler_id:{controler_id},controler_name:{controler_name},channel:{channel},oauth_info:{oauth_info}')
            return f"controler_id:{controler_id},controler_name:{controler_name} is update"
        else:
            oauth_info = OauthCredentialsTable(controler_id=controler_id,controler_name=controler_name,channel=channel,oauth_credentials=oauth_info)
            session.add(oauth_info)
            logger.info_database('update_oauth_info insert success!')
            logger.debug(f'update_oauth_info,controler_id:{controler_id},controler_name:{controler_name},channel:{channel},oauth_info:{oauth_info}')
