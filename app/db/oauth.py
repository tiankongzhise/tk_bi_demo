...
def query_oauth_by_controler_id(controler_id:str,channel:str):
    with db_client.session_scope as session:
        stmt = select(OauthInfoTable).where(OauthInfoTable.controler_id == controler_id and OauthInfoTable.channel == channel)
...
def query_oauth_by_controler_name(controler_name:str,channel:str):
    with db_client.session_scope as session:
        stmt = select(OauthInfoTable).where(OauthInfoTable.controler_name == controler_name and OauthInfoTable.channel == channel)
        oauth_info = session.execute(stmt).scalar_one_or_none()
